...
#$ -e /home/postgrads/2488158T/split_align_merge.err
...
merge_alignments() {
    ...
    cat ${aligned_dir}/*.aln > $merged_output
}

# Check if all sequences are aligned before merging
check_alignment_complete() {
    local num_split_files=$(ls ${split_dir}/*.fa | wc -l)
    local num_aligned_files=$(ls ${aligned_dir}/*.aln 2>/dev/null | wc -l)
    if [[ $num_aligned_files -ge $num_split_files ]]; then
        return 0  # All alignments are complete
    else
        return 1  # Not all alignments are complete
    fi
}
...
# Ensure all sequences are aligned before merging
if check_alignment_complete; then
    # Merge all aligned files into one
    merge_alignments
    echo "Alignment and merging completed."
    echo "Done."
else
    echo "Not all sequences were aligned successfully."
fi
